...
const steps = [
  {
    num: "01", title: "Needs Analysis", subtitle: "Requirements Analysis",
  ...
  {
    num: "02", title: "Market Mapping", subtitle: "Market Positioning",
  ...
  {
    num: "03", title: "Candidate Sourcing", subtitle: "Talent Sourcing",
  ...
  {
    num: "05", title: "Offer & Onboarding", subtitle: "Onboarding",
  ...
  {
    num: "06", title: "Post-Placement", subtitle: "Long-Term Support",
...
/* Pre-computed tick positions for SSR stability */
...
export default function ServiceProcess() {
  ...
  return (
    <section
      ...
      style={{ height: "100dvh", background: "#061410" }}
    >
      {/* ═══ Background layers ═══ */}
      {steps.map((s, i) => (
        <div
      ...
      <div className="absolute top-[5%] left-[5%] md:left-[6%] z-30">
        <p className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.25em] text-white/35 mb-2 md:mb-3">
          Our Process
        </p>
        <h2
      ...
      {/* ═══ WHEEL — desktop: large ring with orbiting nodes ═══ */}
      <div
          ...
          return (
            ...
            >
              <div
                ...
                style={{
                  border: isTop ? `2px solid ${s.color}` : "1px solid rgba(255,255,255,0.12)",
                  boxShadow: isTop ? `0 0 28px ${s.color}40` : "none",
                  background: isTop ? `${s.color}15` : "rgba(255,255,255,0.03)",
                }}
              />
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-25 text-center w-[88%] max-w-[320px] md:max-w-sm px-2 md:px-4 pointer-events-none">
        {/* Big faded number */}
        <p
          className="text-7xl md:text-8xl font-black tracking-tighter mb-1 md:mb-2 select-none"
          style={{ fontFamily: "var(--hg-font-heading)", color: step.color, opacity: 0.15 }}
        >
          {step.num}
        </p>
        ...
        <img
          src={step.icon}
          alt=""
          className="hidden md:block w-14 h-14 object-contain mx-auto mb-4"
          style={{ filter: "brightness(0) invert(1)", opacity: 0.8, pointerEvents: "auto" }}
        />
        ...
        {/* Subtitle */}
        <p className="text-[11px] font-bold uppercase tracking-[0.2em] mb-3" style={{ color: step.color }}>
          {step.subtitle}
        </p>
        ...
        <Link
          href={step.ctaHref}
          className="pointer-events-auto inline-flex items-center gap-2 text-sm font-semibold hover:gap-3 transition-all"
        ...
        {steps.map((s, i) => {
          ...
          return (
            <button
              key={s.num}
              onClick={() => jumpTo(i)}
              className="transition-all duration-300 rounded-full flex items-center gap-1 px-2.5 py-1.5"
              style={{
            ...
            >
              <span
                ...
                style={{ fontFamily: "var(--hg-font-heading)", color: isActive ? s.color : "rgba(255,255,255,0.3)" }}
              >
                {s.num}
              </span>
              {isActive && (
