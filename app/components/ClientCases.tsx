...
const cases = [
  {
    ...
    location: "San Jose → Shanghai",
    result: "Placed in 28 days",
    quote:
      "Happy Global understood both Silicon Valley engineering culture and what it takes to build a world-class team in China. The candidate has since built a 40-person R&D team and shipped three major product releases.",
  ...
  },
  {
    ...
    highlights: ["Dual SEC + FCA expertise", "3 pre-vetted finalists", "14-day delivery"],
    stat: { value: "14", suffix: "d", label: "Shortlist" },
    accent: "#14b8a6",
  },
  {
    id: 3,
    role: "Director, Clinical Ops",
    ...
    highlights: ["FDA submission expertise", "200-candidate market map", "2+ year retention"],
  ...
  },
  {
    ...
    client: "Board Director",
    highlights: ["Trilingual ENG + DE + ZH", "Tier-1 automotive", "+40% revenue growth"],
    ...
    >
      ...
      <div className="absolute top-[7%] left-0 right-0 z-20 text-center">
        <span className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] mb-3 px-4 py-1.5 rounded-full"
          style={{ color: activeCase.accent, background: `${activeCase.accent}0D`, border: `1px solid ${activeCase.accent}1A` }}>
          Client Success Story
        </span>
        <h2 className="text-2xl md:text-3xl font-bold tracking-tight hg-heading">
          Proven Placements,{" "}
          <span style={{ color: activeCase.accent }}>Real Impact</span>
        </h2>
      </div>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-[90%] max-w-4xl pointer-events-none">
        <div
          className="relative rounded-3xl overflow-hidden transition-all duration-[400ms] ease-[cubic-bezier(0.25,0.1,0.25,1)]"
          style={{
            background: "#fff",
        ...
        >
          ...
          <div className="p-8 md:p-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-7">
              <div className="flex-1">
                <span className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] mb-2 px-3 py-1 rounded-md"
                  style={{ color: activeCase.accent, background: `${activeCase.accent}10`, border: `1px solid ${activeCase.accent}1A` }}>
                  {activeCase.industry}
                </span>
              ...
              {/* Stat badge */}
              <div className="shrink-0 text-center rounded-2xl p-5 min-w-[130px]"
                style={{ background: `${activeCase.accent}08`, border: `1px solid ${activeCase.accent}15` }}>
                <p className="text-4xl md:text-5xl font-black hg-heading" style={{ color: activeCase.accent, letterSpacing: "-0.03em" }}>
                  {animatedVal}<span className="text-lg md:text-xl align-top ml-0.5 opacity-60">{activeCase.stat.suffix}</span>
                </p>
            ...
            <div className="relative mb-6">
              <svg className="absolute top-0 left-0 w-10 h-10 opacity-[0.05]" viewBox="0 0 24 24" fill="currentColor" style={{ color: activeCase.accent }}>
                <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
              </svg>
              <p className="text-base md:text-lg leading-relaxed italic pl-14" style={{ color: "var(--hg-color-text-secondary)" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-7">
              {activeCase.highlights.map((h, j) => (
                <div key={j} className="flex items-center gap-2 text-xs font-semibold" style={{ color: "var(--hg-color-text-primary)" }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={activeCase.accent} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6" style={{ borderTop: "1px solid var(--hg-color-border)" }}>
              <div>
                <p className="text-sm font-bold hg-heading">{activeCase.client}</p>
                <p className="text-[11px] mt-0.5" style={{ color: "var(--hg-color-text-secondary)" }}>{activeCase.company}</p>
              </div>
      ...
      <div className="absolute bottom-[5%] left-1/2 -translate-x-1/2 z-30 flex items-center gap-5 pointer-events-auto">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "var(--hg-color-text-secondary)" }}>
          <span style={{ color: activeCase.accent, fontSize: "1.1em" }}>0{active + 1}</span>
          <span className="opacity-25"> / 0{TOTAL}</span>
        </span>
        <div className="flex items-center gap-2">
          {cases.map((c, i) => (
            <button key={c.id} onClick={() => goTo(i)}
              className="rounded-full transition-all duration-500"
              style={{ width: i === active ? "28px" : "7px", height: "7px",
                background: i === active ? c.accent : "var(--hg-color-border)",
                boxShadow: i === active ? `0 0 8px ${c.accent}40` : "none" }}
      ...
      {/* Scroll hint */}
      <div className="absolute bottom-[5%] right-[4%] z-30 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.15em] pointer-events-none"
        style={{ color: "var(--hg-color-text-secondary)", opacity: 0.35 }}>
